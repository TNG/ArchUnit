...
public class JavaTypeTraversalTest {
    ...
    public void traverses_simple_class() {
        ...
        clazz.traverseSignature(newTrackingVisitor(traversedClasses));
    ...
    public void traverses_array_type() {
        ...
        List<JavaType> traversedClasses = new ArrayList<>();
        arrayType.traverseSignature(newTrackingVisitor(traversedClasses));

        assertThatTypes(traversedClasses).matchExactly(
                SimpleClass[][].class,
                SimpleClass[].class,
                SimpleClass.class
        );
    ...
    public void traverses_type_variable() {
        ...
        assertThat(traversedTypes).containsOnly(
                typeVariable,
                typeVariable.getBounds().get(0) // implicitly java.lang.Object
        );
    }
    ...
    public void traverses_parameterized_type() {
        ...
        assertThat(traversedTypes).containsOnly(
                parameterizedType,
                getOnlyElement(parameterizedType.getActualTypeArguments())
        );
    }
    ...
    public void traverses_wildcard_type() {
        ...
        List<JavaType> traversedTypes = new ArrayList<>();
        wildcardType.traverseSignature(newTrackingVisitor(traversedTypes));
    ...
    public void traverses_generic_array_type() {
        ...
        List<JavaType> traversedTypes = new ArrayList<>();
        genericArrayType.traverseSignature(newTrackingVisitor(traversedTypes));

        assertThat(traversedTypes).containsOnly(
                genericArrayType,
                genericArrayType.getComponentType(),
                ((JavaTypeVariable<?>) genericArrayType.getComponentType()).getBounds().get(0) // implicitly java.lang.Object
        );
    }
    ...
    public void traverses_complex_signature_of_JavaClass() {
        ...
        JavaClass primitiveArrayType2DimInt = types.expect(listWildCard.getLowerBounds().get(0));
        JavaClass primitiveArrayType1DimInt = types.expect(primitiveArrayType2DimInt.getComponentType());
        types.expect(primitiveArrayType1DimInt.getComponentType());
        // Type variable V
    ...
    @Test
    public void traverses_complex_signature_of_array_type() {
        @SuppressWarnings("unused")
        class SomeClass {
            String[][] field;
        }
        ...
        ExpectedTypes types = new ExpectedTypes();
        JavaClass stringArray2Dim = types.expect(arrayType);
        JavaClass stringArray1Dim = types.expect(stringArray2Dim.getComponentType());
        types.expect(stringArray1Dim.getComponentType());
    ...
    private static JavaType.SignatureVisitor newTrackingVisitor(List<JavaType> traversedTypes) {
        return new JavaType.SignatureVisitor() {
            ...
            @Override
            public Result visitArrayType(JavaClass type) {
                traversedTypes.add(type);
                return JavaType.SignatureVisitor.super.visitArrayType(type);
            }
