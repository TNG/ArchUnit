...
public class ImportOptionsTest {
    ...
    private static Stream<Arguments> crossProduct(Stream<ImportOption> importOptionStream, List<Arguments> includeMainFolderInput) {
        List<Arguments> importOptions = importOptionStream.map(Arguments::of).collect(Collectors.toList());
        return Lists.cartesianProduct(importOptions, includeMainFolderInput).stream()
