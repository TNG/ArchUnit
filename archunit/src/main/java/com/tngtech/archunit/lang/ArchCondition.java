...
public class ArchCondition<T> extends AbstractArchCondition<T> {
    ...
    private ArchCondition(Predicate<T> predicate, Message<T> message) {
    ...
    public static class Creator<T> {
        ...
        public ArchCondition<T> withMessage(final String message) {
            return withMessage(new Message<T>() {
                @Override
                public String createFor(T item) {
                    return message;
                }
            });
        }

        public ArchCondition<T> withMessage(Message<T> message) {
            return new ArchCondition<>(violationDetector, message);
        }
    }
}
