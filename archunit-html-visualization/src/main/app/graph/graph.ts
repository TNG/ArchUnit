...
import {Root, RootFactory} from "./node/node";
import {GraphView, GraphViewFactory} from "./graph-view";
...
class Graph {
  ...
  constructor(jsonGraph: JsonGraph, svgDomElement: Element, svgContainerDivDomElement: Element, rootFactory: RootFactory, graphViewFactory: GraphViewFactory, visualizationStyles: VisualizationStyles) {
    this._view = graphViewFactory.getGraphView(svgDomElement, svgContainerDivDomElement);
    this._root = rootFactory.getRoot(jsonGraph.root);
...
interface GraphFactory {
  create(svgElement: Element, svgContainerDivElement: Element): Graph
}

class GraphFactoryImplementation {
  ...
  public create(svgElement: Element, svgContainerDivElement: Element): Graph {
    const visualizationData = this._appContext.visualizationData;
    return new Graph(visualizationData.getJsonGraph(), svgElement, svgContainerDivElement, this._appContext.getRoot(), this._appContext.getGraphView(), this._appContext.visualizationStyles);
  }
}

const init = (appContext: AppContext): GraphFactory => new GraphFactoryImplementation(appContext);

export {init, GraphFactory};
