...
Assertion.addMethod('locatedWithinWithPadding', function (parent, padding) {
  ...
  const distanceToNodeRim = new Vector(node.circleData.relativePosition.x, node.circleData.relativePosition.y).length() + node.circleData.getRadius();
...
Assertion.addMethod('notOverlapWith', function (sibling, padding) {
  const node = this._obj;
  const distanceBetweenMiddlePoints = Vector.between(node.circleData.relativePosition, sibling.circleData.relativePosition).length();
  const radiusSum = node.circleData.getRadius() + sibling.circleData.getRadius();
