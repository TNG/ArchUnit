...
public class ExpectedJUnit6TestFailures extends ExpectedTestFailures<DynamicTest> {

    ExpectedJUnit6TestFailures(Class<?>[] testClasses) {
        super(testClasses);
    }
    ...
    public Stream<DynamicTest> toDynamicTests(Consumer<Runnable> aroundTestInvoke) {
        return testClasses.stream()
                .map(testClass -> new RunnableTest(ExpectedJUnit6TestFailures::runTests, testClass))
                .map(test -> dynamicTest(
                        test.getDisplayName(),
                        () -> aroundTestInvoke.accept(() -> assertActualAndExpectedViolationsMatch(test))));
    }

    public static ExpectedJUnit6TestFailures forTests(Class<?>... testClasses) {
        return new ExpectedJUnit6TestFailures(testClasses);
    }
