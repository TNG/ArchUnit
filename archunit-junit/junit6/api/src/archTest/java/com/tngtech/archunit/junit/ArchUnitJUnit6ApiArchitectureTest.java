...
class ArchUnitJUnit6ApiArchitectureTest {
     ...
     */
    @ArchTest
    static void testee_is_archunit_junit6_api(JavaClasses classes) {
        String actualSource = ArchTest.class.getProtectionDomain().getCodeSource().getLocation().toString();
        if (!actualSource.contains("archunit-junit6-api")) {
            throw new AssertionError("expected archunit-junit6-api in path " + actualSource);
        }
    }
}
