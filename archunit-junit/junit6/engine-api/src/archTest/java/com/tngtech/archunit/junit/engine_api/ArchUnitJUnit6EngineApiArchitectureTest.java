...
class ArchUnitJUnit6EngineApiArchitectureTest {
     ...
     */
    @ArchTest
    static void testee_is_archunit_junit6_engine_api(JavaClasses classes) {
        String actualSource = FieldSelector.class.getProtectionDomain().getCodeSource().getLocation().toString();
        if (!actualSource.contains("archunit-junit6-engine-api")) {
            throw new AssertionError("expected archunit-junit6-engine-api in path " + actualSource);
        }
    }
}
