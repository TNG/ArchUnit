...
class ArchUnitJUnit6EngineArchitectureTest {
     ...
     */
    @ArchTest
    static void testee_is_archunit_junit6_engine(JavaClasses classes) {
        String actualSource = ArchUnitTestEngine.class.getProtectionDomain().getCodeSource().getLocation().toString();
        if (!actualSource.contains("archunit-junit6-engine")) {
            throw new AssertionError("expected archunit-junit6-engine in path " + actualSource);
        }
    }
}
